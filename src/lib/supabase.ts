...
export async function supabaseAuthFetch(path: string, init: RequestInit = {}, accessToken?: string) {
...
export async function supabaseDataFetch(path: string, init: RequestInit = {}, accessToken?: string) {
  const { url, key } = getConfig();
  const headers = new Headers(init.headers);
  headers.set("apikey", key);
  headers.set("Content-Type", "application/json");
...
export async function supabaseStorageFetch(path: string, init: RequestInit = {}, accessToken?: string) {
  const { url, key } = getConfig();
  const headers = new Headers(init.headers);
  headers.set("apikey", key);
  headers.set("Authorization", `Bearer ${accessToken || key}`);
...
export function supabasePublicStorageUrl(bucket: string, path: string) {
  const { url } = getConfig();
...
export function publicUser(user: SupabaseAuthUser): PublicUser {
  ...
  const name = typeof rawName === "string" && rawName.trim() ? rawName.trim() : user.email?.split("@")[0] || "Customer";
  return { id: user.id, name, email: user.email || "" };
}

export function setSessionCookies(response: NextResponse, session: AuthSession) {
  response.cookies.set(ACCESS_COOKIE, session.access_token, { httpOnly: true, secure: process.env.NODE_ENV === "production", sameSite: "lax", path: "/", maxAge: session.expires_in || ACCESS_MAX_AGE });
  response.cookies.set(REFRESH_COOKIE, session.refresh_token, { httpOnly: true, secure: process.env.NODE_ENV === "production", sameSite: "lax", path: "/", maxAge: REFRESH_MAX_AGE });
}
...
async function refreshSession(refreshToken: string) {
  const response = await supabaseAuthFetch("/token?grant_type=refresh_token", { method: "POST", body: JSON.stringify({ refresh_token: refreshToken }) });
  if (!response.ok) return null;
...
export async function resolveSession() {
  ...
  const user = refreshedSession.user || (await userFromAccessToken(refreshedSession.access_token));
  if (!user) return null;
  return { user, accessToken: refreshedSession.access_token, refreshedSession };
}
